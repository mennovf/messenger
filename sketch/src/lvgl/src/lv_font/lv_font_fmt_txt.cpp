...
static int8_t get_kern_value(const lv_font_t * font, uint32_t gid_left, uint32_t gid_right)
{
    ...
    if(fdsc->kern_classes == 0) {
        ...
        uint8_t left_class = pgm_read_byte(kdsc->left_class_mapping + gid_left);
        uint8_t right_class = pgm_read_byte(kdsc->left_class_mapping + gid_right);

        /* If class = 0, kerning not exist for that glyph
         * else got the value form `class_pair_values` 2D array*/
        if(left_class > 0 && right_class > 0) {
            uint8_t v = pgm_read_byte(kdsc->class_pair_values + ((left_class-1)* kdsc->right_class_cnt + (right_class-1)));
            value = *(int8_t *)(&v);
        }
